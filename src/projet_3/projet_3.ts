...
let machine_j: number = NaN
let machine_n: number = NaN
let machine_mini: number = NaN

// Variables pour suivre quelles variables sont consultées/modifiées
let variablesModifiees: string[] = []
let variablesConsultees: string[] = []
...
function afficherVariable(x: number, y: number, valeur: string, titre: string, nomVar: string) {
...
function afficherCelluleTableau(x: number, y: number, valeur: string, titre: string, index: number) {
    const varWidth = 60, varHeight = 30
    const valX = 20, valY = 20
    const titreX = 20, titreY = 45

    // Déterminer si cette cellule est consultée/modifiée
    const estMini = (machine_mini === index)
    const estI = (machine_i === index)
    const estJ = (machine_j === index)
...
function afficherMachineVirtuelle() {
    ...
    const cellWidth = 60, cellHeight = 30
    const spacing = 5
...
const boutonRecommencer = document.getElementById("recommmencer") as HTMLButtonElement
...
const boutonAide = document.getElementById("aide") as HTMLButtonElement;
...
function simulationContinue() {
    if (!simulationArrete) {
        ...
        const vitesse = Math.max(50, 250 - machine_n * 10)
